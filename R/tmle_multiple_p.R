#'  Estimate ATE using from TMLE estimator using cross-fit algorithm (multiple repetition)
#'
...
tmle_multiple_p <- function(data,
  ...
  for(cf in 1:num_cf){
    seed = cf_seed[cf]
    runs <- bind_rows(runs, tmle_single_p(data,
                                          exposure,
                                          outcome,
                                          covarsT,
                                          covarsO,
